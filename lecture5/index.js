...
function thisFunc3(x) {
    var array = [];
    array[0] = x; // 왼쪽 변수를 number로 인식해주세요
}
function homework1(lst) {
...
function chkTeacherSubject(info) {
    if (typeof info.subject === "string") {
        return info.subject;
    }
    else if (Array.isArray(info.subject) &&
        info.subject.length > 0 &&
        info.subject.every(function (value) { return typeof value === "string"; })) {
